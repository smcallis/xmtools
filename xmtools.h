...
//}}}
    ...
    namespace internal {
        // For reference, see "The Complex Householder
        // Transform", by Kuo-Liang Chung and Wen-Ming Yan

        // Apply a Householder col from H to cols of A starting at low
        ...
        void reflect_col_col(
            ...
            htype pivot = H[row*hcols + col];
            if (pivot == 0) return;
            htype scale = 1.0/pivot;
        ...
        void reflect_row_row(
            ...
            (void)hrows; // not needed, but it looks nicer
        ...
        /*
        void reflect_row_col_ (
            const cmatrix_t* H, cmatrix_t* A, int64 row, int64 col
        ) {
            cdouble_t pivot = cmatrix_get(H, row, col);
            if (pivot.re == 0 && pivot.im == 0) return;
            for (int64 jj = 0; jj<A->cols; jj++) {
                cdouble_t sum = {};
                for (int64 ii = col; ii<A->rows; ii++) {
                    sum = cdouble_add(sum, cdouble_mul(
                        cmatrix_get(A, ii, jj),
                        cmatrix_get(H, row, ii)
                    ));
                }
                sum = cdouble_div(sum, pivot);
                for (int64 ii = col; ii<A->rows; ii++) {
                    cmatrix_set(A, ii, jj, cdouble_add(
                        cmatrix_get(A, ii, jj),
                        cdouble_cmul(sum, cmatrix_get(H, row, ii))
                    ));
                }
            }
        }
